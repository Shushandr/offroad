...
public class CurrentPositionHelper {
	...
	private void initCtx(OsmWindow app) {
		...
		ctx = new RoutePlannerFrontEnd(false).buildRoutingContext(cfg, app.getResourceManager().getRoutingMapFiles());
		RoutingConfiguration defCfg = app.getDefaultRoutingConfig().build(GeneralRouterProfile.CAR.name().toLowerCase(), 10, 
				new HashMap<String, String>());
		defCtx = new RoutePlannerFrontEnd(false).buildRoutingContext(defCfg, app.getResourceManager().getRoutingMapFiles());
	}
