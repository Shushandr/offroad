...
//		private boolean orientationPortrait;
		...
		public RulerWidget(final OsmWindow app, OsmBitmapPanel ma) {
			this.ma = ma;
			mRulerIcon = app.readImage("ruler.9");
			mRulerNightIcon = app.readImage("ruler_night.9");
...
//			orientationPortrait = AndroidUiHelper.isOrientationPortrait(ma);
		...
		public void updateTextSize(boolean isNight, int textColor, int textShadowColor, int shadowRadius) {
			mRulerPaint.setColor(textColor);
			mRulerShadowPaint.setColor(textShadowColor);
...
//			icon.setBackgroundResource(isNight ? R.drawable.ruler_night : R.drawable.ruler);
		...
		public boolean updateInfo(Graphics2D pCanvas, RotatedTileBox tb, DrawSettings nightMode) {
			boolean visible = true;
			maxWidth= ma.getWidth()/4;
			// update cache
//			if (view.isZooming()) {
//				visible = false;
...
//				visible = false;
//			} else 
//				if ((tb.getZoom() != cacheRulerZoom || Math.abs(tb.getCenterTileX() - cacheRulerTileX) > 1 || Math
//					.abs(tb.getCenterTileY() - cacheRulerTileY) > 1) && tb.getPixWidth() > 0 && maxWidth > 0) {
				...
				double roundedDist = OsmAndFormatter.calculateRoundedDist(maxWidth / 
						pixDensity, ma.getApplication());

				int cacheRulerDistPix = (int) (pixDensity * roundedDist);
				cacheRulerText = OsmAndFormatter.getFormattedDistance((float) roundedDist, ma.getApplication());
//				System.out.println("RULER: " + cacheRulerText + ", " + cacheRulerDistPix);
...
//				layout.requestLayout();
				BufferedImage icon = mRulerIcon;
				if(nightMode.isNightMode()){
					icon = mRulerNightIcon;
				}
				Rectangle rect = new Rectangle(cacheRulerDistPix, icon.getHeight());
				rect.translate(100, ma.getHeight()-100);
//				pCanvas.drawImage(icon, rect.x, rect.y, rect.getMaxX(), rect.getMaxY(), 0, 0, icon.getWidth(), icon.getHeight(), null);
				mRulerPaint.updateGraphics(pCanvas);
				...
				int stringWidth = pCanvas.getFontMetrics().stringWidth(cacheRulerText);
				pCanvas.drawString(cacheRulerText, midpxX - stringWidth/2, midpxY-5);
//			}
