...
//	private TopTextView streetNameView;
	...
	public MapInfoLayer(OsmBitmapPanel map, RouteLayer layer){
		this.map = map;
		settings = map.getApplication().getSettings();
...
//		MapMarkersWidgetsFactory mwf = map.getMapLayers().getMapMarkersLayer().getWidgetsFactory();
		OsmWindow app = view.getApplication();
...
//		alarmControl.setVisibility(false);
		...
		rulerControl = new RulerWidget(app, map);
...
//		ts.night = nightMode;
		ts.textColor = nightMode ? view.getResources().getColor(R.color.widgettext_night) : R.color.color_black.getRGB();
		// Night shadowColor always use widgettext_shadow_night, same as widget background color for non-transparent
		ts.textShadowColor = nightMode ? view.getResources().getColor(R.color.widgettext_shadow_night) : R.color.color_white.getRGB();
...
//		}
	...
	@Override
	public void onDraw(Graphics2D canvas, RotatedTileBox tileBox, DrawSettings drawSettings) {
...
//		alarmControl.updateInfo(drawSettings);
		rulerControl.updateInfo(canvas, tileBox, drawSettings);
