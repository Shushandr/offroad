...
public class OsmAndFormatter {
	...
	public static String getFormattedSpeed(float metersperseconds, OsmWindow ctx) {
		...
		if (mc == SpeedConstants.KILOMETERS_PER_HOUR) {
			...
			if (minperkm >= 10) {
				return ((int) Math.round(minperkm)) + " " + mc.toShortString();
			} else {
			...
			if (minperm >= 10) {
				return ((int) Math.round(minperm)) + " " + mc.toShortString();
			} else {
			...
			if (metersperseconds >= 10) {
				return ((int) Math.round(metersperseconds)) + " " + SpeedConstants.METERS_PER_SECOND.toShortString();
			}
			// for smaller values display 1 decimal digit x.y km/h, (0.5% precision at 20 km/h)
			int kmh10 = (int) Math.round(metersperseconds * 10f);
			return (kmh10 / 10f) + " " + SpeedConstants.METERS_PER_SECOND.toShortString();
		}
	}
	
	
	public static String toPublicString(CityType t, OsmWindow ctx) {
		switch (t) {
		case CITY:
